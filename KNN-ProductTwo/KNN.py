...
def tranfer(info):
    ...
    ls.append(1.5*float(info[2]))
    ls.append(10*float(info[3]))
    ls.extend(list(mapThree.get(info[4])))
    ls.extend(list(mapFour.get(info[5])))
    ls.append(1.5*float(info[6]))
    ls.append(2*float(info[7]))
    return ls
